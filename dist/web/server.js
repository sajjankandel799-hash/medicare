...
class WebServer {
    ...
    setupMiddleware() {
        ...
        this.app.use((req, res, next) => {
            utils_1.logger.info('web-request', `${req.method} ${req.path}`, {
                ip: req.ip,
    ...
    setupRoutes() {
        ...
        this.app.get('/patient-dashboard.html', (req, res) => {
            res.sendFile(path_1.default.join(__dirname, '../../src/public/patient-dashboard.html'));
        });
        this.app.get('/doctor-dashboard.html', (req, res) => {
            res.sendFile(path_1.default.join(__dirname, '../../src/public/doctor-dashboard.html'));
        });
        this.app.get('/admin-dashboard.html', (req, res) => {
            res.sendFile(path_1.default.join(__dirname, '../../src/public/admin-dashboard.html'));
        });
        // API Routes
        this.app.use('/api', this.createApiRouter());
        // Error handling middleware
        this.app.use((error, req, res, next) => {
            const appError = utils_1.ErrorHandler.handleError('web-error', error);
    ...
    createApiRouter() {
        ...
        router.post('/auth/login', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-login', error);
        ...
        router.post('/auth/register-patient', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-register-patient', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        router.post('/auth/register-doctor', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-register-doctor', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        ...
        router.get('/patients', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-get-patients', error);
        ...
        router.get('/patients/:id', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-get-patient', error);
                res.status(500).json(appError.toErrorResponse());
            }
        });
        router.put('/patients/:id', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-update-patient', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        router.delete('/patients/:id', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-delete-patient', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        // Doctors API
        router.get('/doctors', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-get-doctors', error);
                res.status(500).json(appError.toErrorResponse());
            }
        });
        router.put('/doctors/:id', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-update-doctor', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        router.delete('/doctors/:id', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-delete-doctor', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        // Appointments API
        router.get('/appointments', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-get-appointments', error);
                res.status(500).json(appError.toErrorResponse());
            }
        });
        router.get('/appointments/patient/:patientId', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-get-patient-appointments', error);
                res.status(500).json(appError.toErrorResponse());
            }
        });
        router.get('/appointments/doctor/:doctorId', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-get-doctor-appointments', error);
                res.status(500).json(appError.toErrorResponse());
            }
        });
        router.post('/appointments', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-create-appointment', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        ...
        router.put('/appointments/:id/confirm', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-confirm-appointment', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        router.put('/appointments/:id/complete', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-complete-appointment', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        router.put('/appointments/:id/cancel', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-cancel-appointment', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        router.put('/appointments/:id/postpone', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-postpone-appointment', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        router.put('/appointments/:id/accept-postponement', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-accept-postponement', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        router.put('/appointments/:id/reject-postponement', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-reject-postponement', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        router.put('/appointments/:id', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-update-appointment', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        router.delete('/appointments/:id', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-delete-appointment', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        // Medical Records API
        router.get('/medical-records', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-get-medical-records', error);
                res.status(500).json(appError.toErrorResponse());
            }
        });
        router.post('/medical-records', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-create-medical-record', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        router.delete('/medical-records/:id', async (req, res) => {
            try {
                ...
                const appError = utils_1.ErrorHandler.handleError('api-delete-medical-record', error);
                res.status(400).json(appError.toErrorResponse());
            }
        });
        return router;
    ...
    async start() {
        try {
            utils_1.logger.info('web-server', 'Initializing Hospital Management System...');
            this.services = await (0, initialization_1.initializeSystem)();
            this.authService = new AuthService_1.AuthService(this.services.storageManager);
            this.app.listen(this.port, () => {
                utils_1.logger.info('web-server', `Server running on http://localhost:${this.port}`);
            ...
            const appError = utils_1.ErrorHandler.handleError('web-server-start', error);
            console.error('❌ Failed to start web server:', utils_1.ErrorHandler.getUserFriendlyMessage(appError));
            process.exit(1);
        }
    }
}
