...
export class AppointmentService {
   ...
   */
  async scheduleAppointment(data: Omit<Appointment, 'id' | 'createdAt'>): Promise<Appointment> {
  ...
  async getAppointment(id: string): Promise<Appointment | null> {
    return await this.storage.load<Appointment>(this.COLLECTION, id);
  }
  ...
  async getAppointmentsByPatient(patientId: string): Promise<Appointment[]> {
    const allAppointments = await this.storage.loadAll<Appointment>(this.COLLECTION);
   ...
   */
  async requestPostponement(id: string, newDateTime: string, reason: string): Promise<Appointment> {
    const existingAppointment = await this.getAppointment(id);
   ...
   */
  async acceptPostponement(id: string): Promise<Appointment> {
    const existingAppointment = await this.getAppointment(id);
   ...
   * @throws Error if appointment not found or no postponement request
   */
  async rejectPostponement(id: string): Promise<Appointment> {
    const existingAppointment = await this.getAppointment(id);
   ...
   */
  private validateRequiredFields(data: Omit<Appointment, 'id' | 'createdAt'>): void {
    const requiredFields: (keyof Omit<Appointment, 'id' | 'createdAt' | 'notes'>)[] = [
      'patientId',
      'doctorId',
      'dateTime',
      'status',
    ...
    const validStatuses: Appointment['status'][] = ['scheduled', 'confirmed', 'completed', 'cancelled', 'postponed', 'postpone_requested'];
    if (!validStatuses.includes(data.status)) {
      throw new Error(
        `Invalid status: ${data.status}. Status must be one of: ${validStatuses.join(', ')}`
      );
    }
  }
   ...
   */
  private async validateReferentialIntegrity(patientId: string, doctorId: string): Promise<void> {
